...
public class One extends Room implements Runnable {
	...
	public void initializeNonsolidObjects() {
		...
		addRegion(NPCGenerator.makeSign(15 * 32, 13 * 32, 3, "SIGN", "Welcome to the Rabbit Hills.\nForest Ahead.", getDialogueHandler()));
		addRegion(NPCGenerator.makeSign(97 * 32, 22 * 32, 4, "WARNING", "Be careful of large falls!\nUse the ladder ahead to get back on track.", getDialogueHandler()));
		addRegion(NPCGenerator.makeSign(164 * 32, 14 * 32, 6, "SIGN", "Mr. Scrufflebottom's Abode", getDialogueHandler()));
		addRegion(NPCGenerator.makeSign(143 * 32, 28 * 32, 5, "DANGER", "DANGER!\nSTAY OUT!", getDialogueHandler()));

		addBackgroundObject(new Flag(12 * 32 - 16, 10 * 32 + 2 ,0));
		addRegion(NPCGenerator.makeKnight(12 * 32 - 4, 12 * 32, false, "Greetings! It's good that you've arrived. We, the royal knights will be guiding you through the unfamiliar territory along your quest.", getDialogueHandler()));
		...
		addRegion(NPCGenerator.makeKnight(17 * 32 + 4, 12 * 32, true, "We have already begun to spread out. Time is of the essence, so you best be leaving immediately! Onwards!", getDialogueHandler()));
		addTextLabel(new TextLabel(17 * 32 + 8, 14 * 32 - 20, "Press"));
		addTextLabel(new TextLabel(17 * 32 + 6, 14 * 32 - 6, "\"X\"", false));
		
		addRegion(NPCGenerator.makeKnight(33 * 32, 12 * 32, true, "Don't panic! These rabbits are mostly harmless, they couldn't even hurt you if they tried.", getDialogueHandler()));
		addRegion(NPCGenerator.makeKnight(81 * 32, 14 * 32, false, "Try not to fall off these platforms! You never know what'll be waiting for you down below.", getDialogueHandler()));
		addRegion(NPCGenerator.makeKnight(142 * 32, 12 * 32, true, "Mr. Scrufflebottom's home is just ahead. Maybe he knows something about the kidnapper!", getDialogueHandler()));
		
		addRegion(NPCGenerator.makeKnight(71 * 32, 24 * 32, false, "The king will never find me down here!", getDialogueHandler()));
		addRegion(NPCGenerator.makeKnight(135 * 32, 28 * 32, false, "I really wouldn't be down here if I were you...", getDialogueHandler()));
		addRegion(NPCGenerator.makeKnight(138 * 32, 28 * 32, true, "W-what?! You shouldn't be down here! Get back on track!", getDialogueHandler()));
		
		addRegion(
                        new MrScrufflebottom(
                            180 * 32 - 4, 
                            13 * 32 + 18, 
                            "Kidnapping? I've never heard of any kidnapping!\nNow where did I put my key again?",
                            getDialogueHandler()
                            )
                        );
